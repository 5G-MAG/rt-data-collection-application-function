...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_point_altitude_all_of_t *data_collection_model_point_altitude_all_of_create_ref(const data_collection_model_point_altitude_all_of_t *other)
{
    return reinterpret_cast<data_collection_model_point_altitude_all_of_t*>(new std::shared_ptr<PointAltitude_allOf>(*reinterpret_cast<const std::shared_ptr<PointAltitude_allOf >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_point_altitude_all_of_t *data_collection_model_point_altitude_all_of_create_move(data_collection_model_point_altitude_all_of_t *other)
{
    ...
    std::shared_ptr<PointAltitude_allOf > *obj = reinterpret_cast<std::shared_ptr<PointAltitude_allOf >*>(other);
    if (!*obj) {
        delete obj;
        return NULL;
    }
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_point_altitude_all_of_t *data_collection_model_point_altitude_all_of_copy(data_collection_model_point_altitude_all_of_t *point_altitude_all_of, const data_collection_model_point_altitude_all_of_t *other)
{
    if (point_altitude_all_of) {
        ...
        if (obj) {
            if (other) {
                const std::shared_ptr<PointAltitude_allOf > &other_obj = *reinterpret_cast<const std::shared_ptr<PointAltitude_allOf >*>(other);
                if (other_obj) {
                    *obj = *other_obj;
                } else {
                    obj.reset();
            ...
            if (other) {
                const std::shared_ptr<PointAltitude_allOf > &other_obj = *reinterpret_cast<const std::shared_ptr<PointAltitude_allOf >*>(other);
                if (other_obj) {
                    obj.reset(new PointAltitude_allOf(*other_obj));
                } /* else already null shared pointer */
            } /* else already null shared pointer */
        }
    } else {
        point_altitude_all_of = data_collection_model_point_altitude_all_of_create_copy(other);
    }
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_point_altitude_all_of_toJSON(const data_collection_model_point_altitude_all_of_t *point_altitude_all_of, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_point_altitude_all_of_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_point_altitude_all_of_is_equal_to(const data_collection_model_point_altitude_all_of_t *first, const data_collection_model_point_altitude_all_of_t *second)
{
    ...
    const std::shared_ptr<PointAltitude_allOf > &obj2 = *reinterpret_cast<const std::shared_ptr<PointAltitude_allOf >*>(second);
    if (!first) {
        if (!obj2) return true;
        return false;
    }
    const std::shared_ptr<PointAltitude_allOf > &obj1 = *reinterpret_cast<const std::shared_ptr<PointAltitude_allOf >*>(first);
    if (!second) {
        if (!obj1) return true;
        return false;
    }
...
{
    if (!obj_point_altitude_all_of) return 0l;
    std::shared_ptr<PointAltitude_allOf > &obj = *reinterpret_cast<std::shared_ptr<PointAltitude_allOf >*>(obj_point_altitude_all_of);
