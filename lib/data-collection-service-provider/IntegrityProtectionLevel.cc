...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_integrity_protection_level_t *data_collection_model_integrity_protection_level_create_ref(const data_collection_model_integrity_protection_level_t *other)
{
    return reinterpret_cast<data_collection_model_integrity_protection_level_t*>(new std::shared_ptr<IntegrityProtectionLevel>(*reinterpret_cast<const std::shared_ptr<IntegrityProtectionLevel >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_integrity_protection_level_toJSON(const data_collection_model_integrity_protection_level_t *integrity_protection_level, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_integrity_protection_level_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
