...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_media_streaming_session_identification_t *data_collection_model_media_streaming_session_identification_create_ref(const data_collection_model_media_streaming_session_identification_t *other)
{
    return reinterpret_cast<data_collection_model_media_streaming_session_identification_t*>(new std::shared_ptr<MediaStreamingSessionIdentification>(*reinterpret_cast<const std::shared_ptr<MediaStreamingSessionIdentification >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_media_streaming_session_identification_toJSON(const data_collection_model_media_streaming_session_identification_t *media_streaming_session_identification, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_media_streaming_session_identification_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
