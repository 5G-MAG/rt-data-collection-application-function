...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_events_subs_t *data_collection_model_events_subs_create_ref(const data_collection_model_events_subs_t *other)
{
    return reinterpret_cast<data_collection_model_events_subs_t*>(new std::shared_ptr<EventsSubs>(*reinterpret_cast<const std::shared_ptr<EventsSubs >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_events_subs_toJSON(const data_collection_model_events_subs_t *events_subs, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_events_subs_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_events_subs_has_event_rep_info(const data_collection_model_events_subs_t *obj_events_subs)
{
    if (!obj_events_subs) return false;

    const std::shared_ptr<EventsSubs > &obj = *reinterpret_cast<const std::shared_ptr<EventsSubs >*>(obj_events_subs);
    if (!obj) return false;

    return obj->getEventRepInfo().has_value();
}
