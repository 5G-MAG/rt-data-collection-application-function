...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_geographic_area_t *data_collection_model_geographic_area_create_ref(const data_collection_model_geographic_area_t *other)
{
    return reinterpret_cast<data_collection_model_geographic_area_t*>(new std::shared_ptr<GeographicArea>(*reinterpret_cast<const std::shared_ptr<GeographicArea >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_geographic_area_toJSON(const data_collection_model_geographic_area_t *geographic_area, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_geographic_area_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API const data_collection_model_geographical_coordinates_t* data_collection_model_geographic_area_get_entry_point_list(const data_collection_model_geographic_area_t *obj_geographic_area, size_t idx)
{
    if (!obj_geographic_area) {
        const data_collection_model_geographical_coordinates_t *result = NULL;
    ...
    const GeographicArea::PointListType &container = obj->getPointList();
    ...
    typedef typename GeographicArea::PointListItemType ResultFromType;
