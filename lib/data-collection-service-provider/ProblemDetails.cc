...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_problem_details_t *data_collection_model_problem_details_create_ref(const data_collection_model_problem_details_t *other)
{
    return reinterpret_cast<data_collection_model_problem_details_t*>(new std::shared_ptr<ProblemDetails>(*reinterpret_cast<const std::shared_ptr<ProblemDetails >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_problem_details_toJSON(const data_collection_model_problem_details_t *problem_details, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_problem_details_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
