...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_protocol_description_t *data_collection_model_protocol_description_create_ref(const data_collection_model_protocol_description_t *other)
{
    return reinterpret_cast<data_collection_model_protocol_description_t*>(new std::shared_ptr<ProtocolDescription>(*reinterpret_cast<const std::shared_ptr<ProtocolDescription >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_protocol_description_toJSON(const data_collection_model_protocol_description_t *protocol_description, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_protocol_description_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
