...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_umt_location_area5_g_all_of_t *data_collection_model_umt_location_area5_g_all_of_create_ref(const data_collection_model_umt_location_area5_g_all_of_t *other)
{
    return reinterpret_cast<data_collection_model_umt_location_area5_g_all_of_t*>(new std::shared_ptr<UmtLocationArea5G_allOf>(*reinterpret_cast<const std::shared_ptr<UmtLocationArea5G_allOf >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_umt_location_area5_g_all_of_toJSON(const data_collection_model_umt_location_area5_g_all_of_t *umt_location_area5_g_all_of, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_umt_location_area5_g_all_of_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
