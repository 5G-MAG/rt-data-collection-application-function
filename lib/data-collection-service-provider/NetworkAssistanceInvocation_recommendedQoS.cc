...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_network_assistance_invocation_recommended_qo_s_t *data_collection_model_network_assistance_invocation_recommended_qo_s_create_ref(const data_collection_model_network_assistance_invocation_recommended_qo_s_t *other)
{
    return reinterpret_cast<data_collection_model_network_assistance_invocation_recommended_qo_s_t*>(new std::shared_ptr<NetworkAssistanceInvocation_recommendedQoS>(*reinterpret_cast<const std::shared_ptr<NetworkAssistanceInvocation_recommendedQoS >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_network_assistance_invocation_recommended_qo_s_toJSON(const data_collection_model_network_assistance_invocation_recommended_qo_s_t *network_assistance_invocation_recommended_qo_s, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_network_assistance_invocation_recommended_qo_s_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
