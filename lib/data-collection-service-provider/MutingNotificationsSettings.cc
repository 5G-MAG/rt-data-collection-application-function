...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_muting_notifications_settings_t *data_collection_model_muting_notifications_settings_create_ref(const data_collection_model_muting_notifications_settings_t *other)
{
    return reinterpret_cast<data_collection_model_muting_notifications_settings_t*>(new std::shared_ptr<MutingNotificationsSettings>(*reinterpret_cast<const std::shared_ptr<MutingNotificationsSettings >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_muting_notifications_settings_toJSON(const data_collection_model_muting_notifications_settings_t *muting_notifications_settings, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_muting_notifications_settings_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
