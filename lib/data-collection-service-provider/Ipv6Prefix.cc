...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_ipv6_prefix_t *data_collection_model_ipv6_prefix_create_ref(const data_collection_model_ipv6_prefix_t *other)
{
    return reinterpret_cast<data_collection_model_ipv6_prefix_t*>(new std::shared_ptr<Ipv6Prefix>(*reinterpret_cast<const std::shared_ptr<Ipv6Prefix >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_ipv6_prefix_toJSON(const data_collection_model_ipv6_prefix_t *ipv6_prefix, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_ipv6_prefix_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
