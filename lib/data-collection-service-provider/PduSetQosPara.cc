...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_pdu_set_qos_para_t *data_collection_model_pdu_set_qos_para_create_ref(const data_collection_model_pdu_set_qos_para_t *other)
{
    return reinterpret_cast<data_collection_model_pdu_set_qos_para_t*>(new std::shared_ptr<PduSetQosPara>(*reinterpret_cast<const std::shared_ptr<PduSetQosPara >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_pdu_set_qos_para_toJSON(const data_collection_model_pdu_set_qos_para_t *pdu_set_qos_para, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_pdu_set_qos_para_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
