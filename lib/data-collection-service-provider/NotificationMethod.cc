...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_notification_method_t *data_collection_model_notification_method_create_ref(const data_collection_model_notification_method_t *other)
{
    return reinterpret_cast<data_collection_model_notification_method_t*>(new std::shared_ptr<NotificationMethod>(*reinterpret_cast<const std::shared_ptr<NotificationMethod >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_notification_method_toJSON(const data_collection_model_notification_method_t *notification_method, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_notification_method_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
