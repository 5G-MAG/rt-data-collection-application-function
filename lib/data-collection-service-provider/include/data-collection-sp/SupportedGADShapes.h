#ifndef _DATA_COLLECTION_SUPPORTED_GAD_SHAPES_H_
#define _DATA_COLLECTION_SUPPORTED_GAD_SHAPES_H_

/**********************************************************************************************************************************
 * SupportedGADShapes - Public C interface to the SupportedGADShapes object
 **********************************************************************************************************************************
 * License: 5G-MAG Public License (v1.0)
 * Authors: David Waring <david.waring2@bbc.co.uk>
 * Copyright: (C) 2024 British Broadcasting Corporation
 *
 * For full license terms please see the LICENSE file distributed with this
 * program. If this file is missing then the license can be retrieved from
 * https://drive.google.com/file/d/1cinCiA778IErENZ3JN52VFW-1ffHpx7Z/view
 **********************************************************************************************************************************/

#ifndef INCLUDED_FROM_DATA_COLLECTION_H
#error "This file can only be included from data-collection.h"
#endif

#include "SupportedGADShapes_anyOf.h"

/** \addtogroup dcsp_model
 * @{
 */

#ifdef __cplusplus
extern "C" {
#endif

/** A 3GPP SupportedGADShapes object reference
 */
typedef struct data_collection_model_supported_gad_shapes_s
#ifdef DOXYGEN_ONLY
{
    int dummy; /**< \private placeholder */
}
#endif
data_collection_model_supported_gad_shapes_t;


/** SupportedGADShapes enumerated values */
typedef enum {
    DCM_SUPPORTED_GAD_SHAPES_NO_VAL, /**< No value */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POINT, /**< POINT */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POINT_UNCERTAINTY_CIRCLE, /**< POINT_UNCERTAINTY_CIRCLE */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POINT_UNCERTAINTY_ELLIPSE, /**< POINT_UNCERTAINTY_ELLIPSE */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POLYGON, /**< POLYGON */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POINT_ALTITUDE, /**< POINT_ALTITUDE */
    DCM_SUPPORTED_GAD_SHAPES_VAL_POINT_ALTITUDE_UNCERTAINTY, /**< POINT_ALTITUDE_UNCERTAINTY */
    DCM_SUPPORTED_GAD_SHAPES_VAL_ELLIPSOID_ARC, /**< ELLIPSOID_ARC */
    DCM_SUPPORTED_GAD_SHAPES_VAL_LOCAL_2_D_POINT_UNCERTAINTY_ELLIPSE, /**< LOCAL_2D_POINT_UNCERTAINTY_ELLIPSE */
    DCM_SUPPORTED_GAD_SHAPES_VAL_LOCAL_3_D_POINT_UNCERTAINTY_ELLIPSOID, /**< LOCAL_3D_POINT_UNCERTAINTY_ELLIPSOID */
    DCM_SUPPORTED_GAD_SHAPES_VAL_DISTANCE_DIRECTION, /**< DISTANCE_DIRECTION */
    DCM_SUPPORTED_GAD_SHAPES_VAL_RELATIVE_2_D_LOCATION_UNCERTAINTY_ELLIPSE, /**< RELATIVE_2D_LOCATION_UNCERTAINTY_ELLIPSE */
    DCM_SUPPORTED_GAD_SHAPES_VAL_RELATIVE_3_D_LOCATION_UNCERTAINTY_ELLIPSOID, /**< RELATIVE_3D_LOCATION_UNCERTAINTY_ELLIPSOID */
    DCM_SUPPORTED_GAD_SHAPES_OTHER  /**< Custom value (future expansion) */
} data_collection_model_supported_gad_shapes_e;


/** Create a new SupportedGADShapes.
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * @return a new SupportedGADShapes object pointer.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_create();

/** Create a new SupportedGADShapes reference.
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Creates a new reference to the same underlying object as \a other.
 * @param other The SupportedGADShapes to create a new reference to.
 * @return a new reference to the underlying object of \a other.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_create_ref(const data_collection_model_supported_gad_shapes_t *other);

/** Create a new copy of a SupportedGADShapes object.
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Creates a new copy of the given @a other object
 * @param other The SupportedGADShapes to copy.
 * @return a new SupportedGADShapes object pointer.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_create_copy(const data_collection_model_supported_gad_shapes_t *other);

/** Create a new reference of a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Creates a reference to the same underlying @a other object.
 * @param other The SupportedGADShapes to create a new reference to.
 * @return a new SupportedGADShapes object pointer reference.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_create_move(data_collection_model_supported_gad_shapes_t *other);

/** Copy the value of another SupportedGADShapes into this object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Copies the value of @a other {{classname} object into @a supported_gad_shapes.
 * @param supported_gad_shapes The SupportedGADShapes object to copy @a other into.
 * @param other The SupportedGADShapes to copy the value from.
 * @return @a supported_gad_shapes.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_copy(data_collection_model_supported_gad_shapes_t *supported_gad_shapes, const data_collection_model_supported_gad_shapes_t *other);

/** Move the value of another SupportedGADShapes into this object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Discards the current value of @a supported_gad_shapes and moves the value of @a other
 * into @a supported_gad_shapes. This will leave @a other as an empty reference.
 *
 * @param supported_gad_shapes The SupportedGADShapes object to move @a other into.
 * @param other The SupportedGADShapes to move the value from.
 *
 * @return @a supported_gad_shapes.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_move(data_collection_model_supported_gad_shapes_t *supported_gad_shapes, data_collection_model_supported_gad_shapes_t *other);

/** Delete a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 * Destroys the reference to the SupportedGADShapes object and frees the value of SupportedGADShapes if this is the last reference.
 *
 * @param supported_gad_shapes The SupportedGADShapes to free.
 */
DATA_COLLECTION_SVC_PRODUCER_API void data_collection_model_supported_gad_shapes_free(data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/** Get a cJSON tree representation of a SupportedGADShapes
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * Create a cJSON tree representation of the SupportedGADShapes object as either an API request or response. This respects fields marked
 * as Read-only (only in responses) or Write-only (only in requests) in the OpenAPI model description. The resulting cJSON tree
 * must be freed by the caller using the cJSON_Delete() function.
 *
 * @param supported_gad_shapes The SupportedGADShapes to represent as a cJSON tree.
 * @param as_request `true` for an API request or `false` for an API response.
 *
 * @return a cJSON tree or `NULL`.
 */
DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_supported_gad_shapes_toJSON(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes, bool as_request);

/** Parse a cJSON tree into a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * Attempts to interpret a cJSON tree as a SupportedGADShapes API request or response (dependent on @a as_request value). If successful
 * will return a new referenced SupportedGADShapes object containing the value represented by the cJSON tree. On failure will return
 * `NULL` and will set @a error_reason, @a error_class and @a error_parameter output parameters to indicate the reason for the
 * failure. The @a error_reason, @a error_class and @a error_parameter strings must be freed using ogs_free().
 *
 * @param json The cJSON tree object to interpret.
 * @param as_request `true` to interpret the @a json as an API request or `false` for interpretation as an API response.
 * @param[out] error_reason On failure will be set to a new nul terminated string indicating the reason for the failure.
 * @param[out] error_class On failure will be set to the class name of the object where the failure happened if available or `NULL`.
 * @param[out] error_parameter On failure will be set to the JSON path of the field where the failure happened if available or
                               `NULL`.
 *
 * @return a new SupportedGADShapes object or `NULL` if the parsing failed.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_t *data_collection_model_supported_gad_shapes_fromJSON(cJSON *json, bool as_request, char **error_reason, char **error_class, char **error_parameter);

/** Compare two SupportedGADShapes objects to see if they are equivalent
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * This will return `true` if the two objects are referencing the same object or the two objects have the same value.
 *
 * @param supported_gad_shapes The first SupportedGADShapes object to compare.
 * @param other_supported_gad_shapes The second SupportedGADShapes object to compare.
 *
 * @return `true` if the two objects are the same or have equal values, `false` otherwise.
 */
DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_supported_gad_shapes_is_equal_to(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes, const data_collection_model_supported_gad_shapes_t *other_supported_gad_shapes);


/** Check if the SupportedGADShapes enumeration object has a value set
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * @param supported_gad_shapes The SupportedGADShapes enumeration object to check.
 * @return `true` if the enumeration object has no value set (i.e. is equivalent to `NULL`)
 */
DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_supported_gad_shapes_is_not_set(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/** Check if the SupportedGADShapes enumeration object has a custom value set
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * @param supported_gad_shapes The SupportedGADShapes enumeration object to check.
 * @return `true` if the enumeration object has an unknown string value set.
 */
DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_supported_gad_shapes_is_non_standard(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/** Get the enumeration value for a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * @param supported_gad_shapes The SupportedGADShapes object to extract the enumeration value from.
 *
 * @return the data_collection_model_supported_gad_shapes_e value represented by the SupportedGADShapes object.
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_supported_gad_shapes_e data_collection_model_supported_gad_shapes_get_enum(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/** Get the string representation of the enumeration value for a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * @param supported_gad_shapes The SupportedGADShapes object to extract the enumeration value from.
 *
 * @return the API string for the enumeration or `NULL` if no value set.
 */
DATA_COLLECTION_SVC_PRODUCER_API const char *data_collection_model_supported_gad_shapes_get_string(const data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/** Set the enumeration value for a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * @param supported_gad_shapes The SupportedGADShapes object to set the enumeration value for.
 * @param value The value to set.
 *
 * @return `true` if the value was set.
 */
DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_supported_gad_shapes_set_enum(data_collection_model_supported_gad_shapes_t *supported_gad_shapes, data_collection_model_supported_gad_shapes_e value);

/** Set the enumeration value as a string for a SupportedGADShapes object
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * Sets the enumeration value from the string given. If the string is a non-standard, custom value then the
 * data_collection_model_supported_gad_shapes_get_enum
 * function will return DCM_SUPPORTED_GAD_SHAPES_OTHER
 * while this value is set to indicate that this is a custom value.
 *
 * @param supported_gad_shapes The SupportedGADShapes object to set the enumeration value for.
 * @param value The value to set.
 *
 * @return `true` to indicate that the value was set.
 */
DATA_COLLECTION_SVC_PRODUCER_API bool data_collection_model_supported_gad_shapes_set_string(data_collection_model_supported_gad_shapes_t *supported_gad_shapes, const char *value);


/** lnode helper for generating ogs_list_t nodes's of type SupportedGADShapes
 * \public \memberof data_collection_model_supported_gad_shapes_t
 *
 * Creates a new data_collection_lnode_t object containing the @a supported_gad_shapes object.
 * The @a supported_gad_shapes will be deleted when the data_collection_lnode_t is freed. Use
 * data_collection_lnode_create_ref() if you want an list node that will not delete the object when it is freed.
 *
 * @param supported_gad_shapes The SupportedGADShapes object to create an data_collection_lnode_t object for.
 *
 * @return a new data_collection_lnode_t object containing the @a supported_gad_shapes
 */
DATA_COLLECTION_SVC_PRODUCER_API data_collection_lnode_t *data_collection_model_supported_gad_shapes_make_lnode(data_collection_model_supported_gad_shapes_t *supported_gad_shapes);

/***** Internal library protected functions *****/

#ifdef __cplusplus
}
#endif

/** @} */

/* vim:ts=8:sts=4:sw=4:expandtab:
 */

#endif /* ifndef _DATA_COLLECTION_SUPPORTED_GAD_SHAPES_H_ */

