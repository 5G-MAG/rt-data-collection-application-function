...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_plmn_id_t *data_collection_model_plmn_id_create_ref(const data_collection_model_plmn_id_t *other)
{
    return reinterpret_cast<data_collection_model_plmn_id_t*>(new std::shared_ptr<PlmnId>(*reinterpret_cast<const std::shared_ptr<PlmnId >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_plmn_id_toJSON(const data_collection_model_plmn_id_t *plmn_id, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_plmn_id_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
