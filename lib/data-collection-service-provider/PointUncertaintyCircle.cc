...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API data_collection_model_point_uncertainty_circle_t *data_collection_model_point_uncertainty_circle_create_ref(const data_collection_model_point_uncertainty_circle_t *other)
{
    return reinterpret_cast<data_collection_model_point_uncertainty_circle_t*>(new std::shared_ptr<PointUncertaintyCircle>(*reinterpret_cast<const std::shared_ptr<PointUncertaintyCircle >*>(other)));
}
...
extern "C" DATA_COLLECTION_SVC_PRODUCER_API cJSON *data_collection_model_point_uncertainty_circle_toJSON(const data_collection_model_point_uncertainty_circle_t *point_uncertainty_circle, bool as_request)
{
    ...
    if (obj) {
        try {
            ...
            ogs_error("Failed to convert data_collection_model_point_uncertainty_circle_t to cJSON [%s.%s]: %s", err.classname.c_str(), err.parameter.c_str(), err.what());
        }
    }
    return NULL;
}
